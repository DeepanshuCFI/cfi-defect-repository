export default function Drawer({ feature, incidents, meta, tierColor, tierText, tierLabel, onClose }) {
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-[26rem] max-w-full bg-white border-l border-line
      ...
      <div className="sticky top-0 bg-white border-b border-line px-5 py-3.5 flex justify-between items-start gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {p.escalation && (
              <span className="font-heading font-bold text-[10px] px-2 py-0.5 rounded-full bg-danger text-white">
        ...
        <button onClick={onClose} className="no-print text-muted hover:text-ink text-xl leading-none cursor-pointer">×</button>
      </div>

      <div className="px-5 py-4 grid grid-cols-3 gap-2 border-b border-line">
      ...
      {(p.defects || []).length > 0 && (
        <div className="px-5 py-3 border-b border-line">
          <div className="font-heading text-[10px] tracking-[0.2em] text-muted mb-1.5">REPORTED DEFECTS</div>
          <div className="flex flex-wrap gap-1.5">
            {p.defects.map(d => (
              <span key={d} className="text-[11.5px] px-2.5 py-1 rounded-full border border-warn/40 bg-warn/5 text-[#a85500]">
      ...
      {bd && (
        <div className="px-5 py-3 border-b border-line">
          <div className="font-heading text-[10px] tracking-[0.2em] text-muted mb-2">
            WHY THIS SCORE <span className="normal-case tracking-normal">(never a black box)</span>
          </div>
          {Object.entries(comps).map(([k, v]) => (
            <div key={k} className="flex items-center gap-2 py-0.5">
              <span className="text-[11.5px] text-muted w-40 shrink-0">{compLabel[k] || k}</span>
              ...
              <span className="text-[11px] text-muted w-8 text-right">{(v * 100).toFixed(0)}</span>
            </div>
          ))}
      ...
      <div className="px-5 py-3">
        ...
        {members.map(i => (
          <div key={i.id} className="rounded-xl border border-line p-3.5 mb-2.5">
            <div className="flex justify-between text-[12px] text-muted">
              <span className="font-semibold text-ink">{i.date || 'undated'}</span>
            ...
            {i.defects.map((d, ix) => (
              <div key={ix} className="mt-2 text-[11.5px] bg-brand-lite rounded-lg px-3 py-2">
                ...
                <div className="text-muted mt-0.5">“{d.evidence}”</div>
              </div>
            ))}
      ...
      <div className="px-5 pb-6 flex gap-2 no-print">
        <a href={correctionMail}
          className="text-[12px] px-4 py-2 rounded-full border border-line text-muted hover:border-brand hover:text-brand">
          Report a correction
        </a>
        <button onClick={() => window.print()}
          className="text-[12px] px-4 py-2 rounded-full border border-line text-muted hover:border-brand hover:text-brand cursor-pointer">
          Print / save PDF
        </button>
      </div>
    </div>
...
function Stat({ n, l, red }) {
  return (
    <div className="rounded-xl border border-line p-2.5 text-center">
